...
describe('deviceProfiles', () => {
	...
	it('buildPrivacyMaskPatch should map to the privacy_mask key', () => {
		expect(buildPrivacyMaskPatch(true)).to.deep.equal({
			patch: { privacy_mask: true },
			value: true,
			error: null,
		});
		expect(buildPrivacyMaskPatch('0')).to.deep.equal({
			patch: { privacy_mask: false },
			value: false,
			error: null,
		});
	});

	it('buildTextOverlayPatch should map to the text_overlay key', () => {
		expect(buildTextOverlayPatch(true)).to.deep.equal({
			patch: { text_overlay: true },
			value: true,
			error: null,
		});
		expect(buildTextOverlayPatch('off')).to.deep.equal({
			patch: { text_overlay: false },
			value: false,
			error: null,
		});
	});

	it('normalizeTextPosition should accept only the known enum values', () => {
		for (const option of TEXT_POSITION_OPTIONS) {
			expect(normalizeTextPosition(option)).to.equal(option);
			expect(normalizeTextPosition(option.toUpperCase())).to.equal(option);
		}
		expect(normalizeTextPosition('bogus')).to.equal(null);
		expect(normalizeTextPosition(undefined)).to.equal(null);
	});

	it('buildLeftTextPatch should return a patch or an error', () => {
		expect(buildLeftTextPatch('camera-name')).to.deep.equal({
			patch: { left_text: 'camera-name' },
			value: 'camera-name',
			error: null,
		});
		const bad = buildLeftTextPatch('bogus');
		expect(bad.patch).to.equal(null);
		expect(bad.error).to.include('invalid leftText');
	});

	it('buildRightTextPatch should return a patch or an error', () => {
		expect(buildRightTextPatch('timestamp')).to.deep.equal({
			patch: { right_text: 'timestamp' },
			value: 'timestamp',
			error: null,
		});
		const bad = buildRightTextPatch('bogus');
		expect(bad.patch).to.equal(null);
		expect(bad.error).to.include('invalid rightText');
	});

	it('buildCustomLeftTextPatch should pass through the string as-is', () => {
		expect(buildCustomLeftTextPatch('Front door')).to.deep.equal({
			patch: { custom_left_text: 'Front door' },
			value: 'Front door',
			error: null,
		});
		expect(buildCustomLeftTextPatch(null)).to.deep.equal({
			patch: { custom_left_text: '' },
			value: '',
			error: null,
		});
	});

	it('buildCustomRightTextPatch should pass through the string as-is', () => {
		expect(buildCustomRightTextPatch('Back yard')).to.deep.equal({
			patch: { custom_right_text: 'Back yard' },
			value: 'Back yard',
			error: null,
		});
	});

	it('normalizeTextScale should round and cap to 1-10', () => {
		expect(normalizeTextScale('5')).to.equal(5);
		expect(normalizeTextScale(2.6)).to.equal(3);
		expect(normalizeTextScale(0)).to.equal(1);
		expect(normalizeTextScale(999)).to.equal(10);
		expect(normalizeTextScale('abc')).to.equal(null);
	});

	it('buildTextScalePatch should return a patch or an error', () => {
		expect(buildTextScalePatch(4)).to.deep.equal({ patch: { text_scale: 4 }, value: 4, error: null });
		const bad = buildTextScalePatch('abc');
		expect(bad.patch).to.equal(null);
		expect(bad.error).to.be.a('string');
	});
});
