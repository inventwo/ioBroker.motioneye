...
/** ioBroker meta file storage lives under `{dataDir}/files/{namespace}/`. */
const IOBROKER_META_FILES_DIR = 'files';
...
/**
 * Absolute filesystem path for Telegram, scripts, etc. (meta storage via adapter writeFile).
 *
 * @param {string} dataDir ioBroker data directory from getAbsoluteDefaultDataDir()
 * @param {string} namespace Adapter namespace, e.g. motioneye.0
 * @param {string} channel Camera channel id
 * @returns {string}
 */
function buildSnapshotAbsoluteFilePath(dataDir, namespace, channel) {
	return path.join(dataDir, IOBROKER_META_FILES_DIR, namespace, buildSnapshotStoragePath(channel));
}
