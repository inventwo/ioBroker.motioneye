...
function createTelegramNotificationManager(deps) {
	 ...
	 */
	async function sendSnapshotImageToRecipient(recipient, camera) {
		...
		await sendToRecipient(recipient, { text: buffer, type });
		deps.verboseLog(`Telegram image sent for "${camera.name}" (${buffer.length} bytes, ${type})`);
		return true;
	}
	...
	async function deliverNotification(camera, notification, filePath, lastUpdate) {
		...
		for (const recipient of recipients) {
			...
			if (notification.sendImage) {
				await sendSnapshotImageToRecipient(recipient, camera);
			} else {
				deps.log('debug', `Telegram image skipped for ${camera.name}: Bild senden = Nein`);
			}
			if (postText) {
