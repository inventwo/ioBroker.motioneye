 ...
 * @param {boolean} [legacyExcluded] Legacy opt-out checkbox
 * @returns {boolean}
 */
...
function resolveCameraNotification(entry) {
	return {
		enabled: resolveNotificationTriState(entry?.notificationEnabled, entry?.notificationExcluded === true),
...
function createTelegramNotificationManager(deps) {
	...
	async function onSnapshotCached(camera, meta) {
		if (deps.isUnloading() || !shouldSendForCamera(camera)) {
			return;
		}
