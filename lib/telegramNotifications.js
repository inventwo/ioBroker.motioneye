...
function resolveCameraNotification(entry) {
	const imageExcluded = entry?.notificationImageExcluded === true || entry?.notificationSendImage === false;
	const timestampExcluded =
		entry?.notificationTimestampExcluded === true || entry?.notificationIncludeTimestamp === false;

	return {
		enabled: entry?.notificationExcluded !== true,
		preText: String(entry?.notificationPreText || ''),
		sendImage: !imageExcluded,
		postText: String(entry?.notificationPostText || ''),
		includeTimestamp: !timestampExcluded,
