...
/**
 * @param {unknown} value Per-trigger select — empty = default yes unless legacy master off
 * @param {boolean} legacyMasterOff Legacy notificationEnabled/notificationExcluded opt-out
 * @returns {boolean}
 */
function resolveNotificationTrigger(value, legacyMasterOff) {
	if (value === 'false' || value === false) {
		return false;
	}
...
function isLegacyNotificationMasterOff(entry) {
	return (
		entry?.notificationExcluded === true ||
		entry?.notificationEnabled === 'false' ||
		entry?.notificationEnabled === false
	);
}
...
function resolveCameraNotification(entry) {
	...
	return {
		onMotion: resolveNotificationTrigger(entry?.notificationOnMotion, legacyMasterOff),
		onSnapshot: resolveNotificationTrigger(entry?.notificationOnSnapshot, legacyMasterOff),
		preText: String(entry?.notificationPreText || ''),
		sendImage: resolveNotificationTriState(entry?.notificationSendImage, entry?.notificationImageExcluded === true),
		postText: String(entry?.notificationPostText || ''),
		includeTimestamp: resolveNotificationTriState(
			entry?.notificationIncludeTimestamp,
			entry?.notificationTimestampExcluded === true,
		),
 ...
 * @returns {boolean}
 */
function shouldNotifyForTrigger(trigger, config) {
	...
	if (trigger === 'motion') {
		return config.telegramNotificationOnMotion !== false;
	}
	if (trigger === 'snapshot' || trigger === 'manual') {
		return config.telegramNotificationOnSnapshot === true;
	}
	return false;
}
...
function createTelegramNotificationManager(deps) {
	...
	async function deliverNotification(camera, notification, filePath, lastUpdate) {
		...
		deps.verboseLog(`Telegram notification sent for "${camera.name}" to ${recipients.length} recipient(s)`);
	}
	...
	async function onSnapshotCached(camera, meta) {
		...
		if (!shouldNotifyForTrigger(meta.trigger, deps.getConfig())) {
			return;
		}
	...
	async function onMotion(camera) {
		...
		if (!shouldNotifyForTrigger('motion', deps.getConfig())) {
			return;
		}
