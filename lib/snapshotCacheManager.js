...
function createSnapshotCacheManager(deps) {
	...
	async function refreshSnapshotNow(camera, trigger, options = {}) {
		...
		const needsMotionEyeSnapshot = trigger === 'notification' || trigger === 'motion';
		if (needsMotionEyeSnapshot) {
			await api.takeSnapshot(camera.motionEyeId);
		}

		if ((trigger === 'snapshot' || needsMotionEyeSnapshot) && delayMs > 0) {
