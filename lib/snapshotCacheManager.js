...
function createSnapshotCacheManager(deps) {
	 ...
	 * @returns {Promise<void>}
	 */
	async function refreshSnapshotNow(camera, trigger) {
		if (deps.isUnloading() || !isEnabledForCamera(camera)) {
			return;
		}
		...
		await deps.setState(`${prefix}.lastUpdate`, new Date(now).toISOString(), true);
	 ...
	 * @returns {Promise<void>}
	 */
	function queueRefresh(camera, trigger) {
		const key = camera.channel;
		const run = async () => {
			try {
				await refreshSnapshotNow(camera, trigger);
			} catch (error) {
	 ...
	 * @returns {Promise<void>}
	 */
	function scheduleAfterSnapshot(camera) {
		return queueRefresh(camera, 'snapshot');
	}
	 ...
	 * @returns {Promise<void>}
	 */
	function refreshManual(camera) {
		return queueRefresh(camera, 'manual');
	}
