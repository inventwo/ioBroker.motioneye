 ...
 * @property {boolean} [storageAutoRefresh] Whether this camera participates in the storage auto-refresh interval (Storage config tab)
 */
 ...
 * @property {boolean} storageAutoRefresh Whether this camera is included in the global storage auto-refresh interval
 */
...
function resolveCameras(cameras, fallbackDefaultMode = 'off', channelNameFn = safeChannelName) {
	...
	for (const entry of cameras) {
		...
		resolved.push({
			...
			storageAutoRefresh: entry.storageAutoRefresh !== false,
		});
	}
