...
const { resolveCameraNotification } = require('./telegramNotifications');
 ...
 *   backfill.
 * @property {boolean} [snapshotCacheExcluded] Opt-out checkbox (Snapshots config tab) — true skips snapshot cache
 *   updates for this camera.
 * @property {string} [notificationPreText] Text sent before the image (Notifications tab)
 * @property {''|'false'} [notificationSendImage] Send image select — empty = yes, "false" = no
 * @property {string} [notificationPostText] Text sent after the image
 * @property {''|'false'} [notificationIncludeTimestamp] Timestamp select — empty = yes, "false" = no
 * @property {''|'false'} [notificationOnMotion] Motion notifications select — empty = yes, "false" = no
 * @property {''|'false'} [notificationOnSnapshot] Snapshot notifications select — empty = yes, "false" = no
 * @property {''|'false'} [notificationEnabled] Legacy notifications select — empty = yes, "false" = both off
 * @property {string} [notificationRecipients] Comma-separated recipient names or chat IDs; empty = all enabled recipients
 * @property {boolean} [notificationImageExcluded] Legacy opt-out checkbox
 * @property {boolean} [notificationTimestampExcluded] Legacy opt-out checkbox
 * @property {boolean} [notificationExcluded] Legacy opt-out checkbox
 */
 ...
 * @property {boolean} snapshotCacheEnabled Whether snapshot cache updates run for this camera (opposite of snapshotCacheExcluded)
 * @property {import('./telegramNotifications').CameraNotificationConfig} notification Per-camera Telegram notification template
 */
...
function resolveCameras(cameras, fallbackDefaultMode = 'off', channelNameFn = safeChannelName) {
	...
	for (const entry of cameras) {
		...
		resolved.push({
			...
			snapshotCacheEnabled: entry.snapshotCacheExcluded !== true,
			notification: resolveCameraNotification(entry),
		});
	}
