...
describe('motionEyeApi downloadPicture', () => {
	const jpeg = Buffer.from([0xff, 0xd8, 0xff, 0xe0, 0x00, 0x10]);

	function startFakePictureServer() {
		const server = http.createServer((req, res) => {
			if (req.url.startsWith('/picture/1/download/lastsnap.jpg')) {
				res.writeHead(200, { 'Content-Type': 'image/jpeg' });
				res.end(jpeg);
				return;
			}
			if (req.url.startsWith('/picture/1/current/')) {
				res.writeHead(200, { 'Content-Type': 'image/jpeg' });
				res.end(jpeg);
				return;
			}
	...
	it('downloadPicture should return JPEG bytes from lastsnap.jpg', async () => {
		const server = await startFakePictureServer();
		...
		try {
			...
			const data = await api.downloadPicture(1);
			expect(Buffer.isBuffer(data)).to.equal(true);
			expect(data.slice(0, 3)).to.deep.equal(jpeg.slice(0, 3));
		} finally {
			server.close();
		}
	});

	it('downloadPicture should fall back to /current/ when lastsnap is missing', async () => {
		const server = http.createServer((req, res) => {
			if (req.url.startsWith('/picture/2/download/lastsnap.jpg')) {
				res.writeHead(404);
				res.end('missing');
				return;
			}
			if (req.url.startsWith('/picture/2/current/')) {
				res.writeHead(200, { 'Content-Type': 'image/jpeg' });
				res.end(jpeg);
				return;
			}
		...
		try {
			...
			const data = await api.downloadPicture(2);
			expect(data.slice(0, 3)).to.deep.equal(jpeg.slice(0, 3));
		} finally {
			server.close();
		}
