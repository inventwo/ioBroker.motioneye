...
function getUnauthorizedVerboseHints(config) {
	...
	lines.push(
		'If web login works but the adapter still fails, check your MotionEye version: 0.44+ uses session login (requires adapter 0.5.0 or newer). On 0.43.x use admin password in motion.conf.',
	);
