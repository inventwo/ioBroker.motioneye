 ...
 * 	'debugging_verbose' | 'motionHost' | 'motionEyePort' | 'motionEyeUser' | 'motionEyePassword'
 * >} DiagLogConfig
 */
...
/**
 * @param {DiagLogConfig | null | undefined} config Adapter native config.
 * @returns {boolean}
 */
...
function getUnauthorizedVerboseHints(config) {
	...
	lines.push(
		'If web login works but the adapter still fails, check your MotionEye version: 0.44+ uses session login (supported from adapter 0.5.0-alpha / latest GitHub). On 0.43.x use admin password in motion.conf.',
	);
