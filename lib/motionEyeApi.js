...
function throwApiError(result, data, method, pathLabel, verboseLog) {
	const message =
		(data && typeof data === 'object' && data !== null && 'error' in data && /** @type {{ error: unknown }} */ (data).error) ||
		result.body ||
		`HTTP ${result.status}`;
	if (verboseLog) {
...
function createMotionEyeApi(options) {
	 ...
	 */
	function httpRequest(path, method = 'GET', body = null, extraHeaders = {}) {
	...
	async function loginSession() {
		...
		const result = await httpRequest('/login', 'POST', body);
	...
	async function requestApi(path, method = 'GET', body = null) {
		...
		if (verboseLog) {
			const hint = data && typeof data === 'object' && data !== null && 'error' in data ? String(data.error) : 'unauthorized';
