...
describe('cameraRegistry', () => {
	...
	it('resolveCameras should default storageAutoRefresh to true and honor explicit storageAutoRefreshExcluded', () => {
		const cameras = resolveCameras([
			{ name: 'Auffahrt', motionEyeId: 1 },
			{ name: 'Carport', motionEyeId: 2, storageAutoRefreshExcluded: true },
			{ name: 'Garten', motionEyeId: 3, storageAutoRefreshExcluded: false },
		]);
		expect(cameras[0].storageAutoRefresh).to.equal(true);
		expect(cameras[1].storageAutoRefresh).to.equal(false);
		expect(cameras[2].storageAutoRefresh).to.equal(true);
	});
