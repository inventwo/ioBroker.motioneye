...
describe('telegramNotifications helpers', () => {
	...
	it('resolveCameraNotification should default onMotion and onSnapshot to true', () => {
		...
		expect(resolveCameraNotification({ notificationEnabled: 'false', notificationOnSnapshot: 'false' })).to.include({
			onSnapshot: false,
		});
		expect(resolveCameraNotification({ notificationImageExcluded: true })).to.include({ sendImage: true });
		expect(resolveCameraNotification({ notificationSendImage: 'false' })).to.include({ sendImage: false });
	...
	it('telegramSnapshotMessageType should use document for very large JPEGs', () => {
		expect(telegramSnapshotMessageType(Buffer.alloc(1024))).to.equal('photo');
		expect(telegramSnapshotMessageType(Buffer.alloc(TELEGRAM_PHOTO_MAX_BYTES + 1))).to.equal('document');
	});
