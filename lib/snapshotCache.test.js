...
describe('snapshotCache helpers', () => {
	...
	it('buildSnapshotAbsoluteFilePath should resolve under iobroker-data/files/{namespace}/', () => {
		const filePath = buildSnapshotAbsoluteFilePath('/opt/iobroker/iobroker-data', 'motioneye.0', 'inventwo');
		expect(filePath).to.equal(
			path.join('/opt/iobroker/iobroker-data', 'files', 'motioneye.0', 'snapshots', 'inventwo', 'lastsnap.jpg'),
		);
	});
