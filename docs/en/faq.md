...
3. **Global auto-refresh switch:** **Settings → Enable storage stats auto-refresh** (`storagePollEnabled`) is off by default. Turn it on and set **Storage stats auto-refresh interval** (`storagePollIntervalSec`, e.g. `3600` for hourly) to refresh cameras automatically on that interval.
4. **Per-camera opt-out:** the **Storage** config tab lists one row per camera with an **Auto-refresh** checkbox (checked by default). Uncheck it for unimportant cameras with large media libraries so they are skipped by the automatic interval — their `storage.refresh` datapoint still works on demand regardless. The same tab has a **"Refresh storage stats now"** button that immediately refreshes every camera in the table (ignoring the checkbox), handy for a one-off update of all cameras without waiting for the interval.
