...
3. **Globaler Auto-Aktualisierung-Schalter:** **Einstellungen → Speicherplatz-Statistik Auto-Aktualisierung aktivieren** (`storagePollEnabled`) ist standardmäßig aus. Aktiviere ihn und stelle **Speicherplatz-Statistik Auto-Aktualisierung (Sekunden)** (`storagePollIntervalSec`, z. B. `3600` für stündlich) ein, damit Kameras automatisch in diesem Intervall aktualisiert werden.
4. **Pro Kamera abwählen:** Der Config-Tab **Storage** zeigt eine Zeile pro Kamera mit einem Häkchen **Auto-Aktualisierung** (standardmäßig aktiviert). Deaktiviere es für unwichtige Kameras mit großen Medienarchiven, damit sie beim automatischen Intervall übersprungen werden — ihr Datenpunkt `storage.refresh` funktioniert davon unabhängig weiterhin jederzeit. Im selben Tab gibt es außerdem den Button **"Speicherplatz-Statistik jetzt aktualisieren"**, der sofort alle Kameras der Tabelle aktualisiert (unabhängig vom Häkchen) — praktisch für eine einmalige Aktualisierung aller Kameras, ohne auf das Intervall zu warten.
